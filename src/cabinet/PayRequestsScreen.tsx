...
export const SELECT44: CSSProperties = { width: '100%', height: 44, border: '1px solid #DFDCD6', borderRadius: 10, padding: '0 34px 0 13px', fontSize: 13.5, background: '#fff', outline: 'none', appearance: 'none', WebkitAppearance: 'none', cursor: 'pointer' };
export const INPUT44: CSSProperties = { width: '100%', height: 44, border: '1px solid #DFDCD6', borderRadius: 10, padding: '0 13px', fontSize: 13.5, background: '#fff', outline: 'none' };
...
export function Select44({ value, onChange, children, style }: {
  ...
  return (
    <div style={{ position: 'relative', ...style }}>
      ...
      <span style={{ position: 'absolute', right: 14, top: 15, pointerEvents: 'none', color: '#A6ACA8' }}>▾</span>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: ACC, color: '#fff', fontSize: 14.5, fontWeight: 600, cursor: disabled ? 'default' : 'pointer', marginTop: 20,
...
export const TH_CAB: CSSProperties = { padding: '11px 14px', fontSize: 10.5, fontWeight: 600, letterSpacing: '.05em', color: '#8A918D', textAlign: 'left', borderBottom: '1px solid #E7E5E0' };
export const TD_CAB: CSSProperties = { padding: '13px 14px', borderBottom: '1px solid #F3F2ED' };
  ...
  return (
    <div data-screen-label="Заявки на оплату" style={{ maxWidth: 1120, animation: 'cabFade .2s ease' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px 22px' }}>
          ...
          <div>
            ...
            <textarea value={name} onChange={(e) => setName(e.target.value)} maxLength={200} placeholder="За что платим (3–200 символов)"
              style={{ width: '100%', height: 88, border: '1px solid #DFDCD6', borderRadius: 10, padding: '11px 13px', fontSize: 13.5, background: '#fff', outline: 'none', resize: 'none', lineHeight: 1.5 }} />
          </div>
          <div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', fontSize: 14, fontWeight: 700, borderBottom: '1px solid #F0EFEA' }}>МОИ ПОСЛЕДНИЕ ЗАЯВКИ</div>
