...
export default function ArticleOpsDrawer(p: ArticleOpsDrawerProps) {
  ...
  return (
    <div data-article-ops style={{ position: 'fixed', inset: 0, zIndex: 66 }}>
      ...
      <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 560, maxWidth: '96vw', background: '#FBFAF7', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,.16)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 20px', background: '#fff', borderBottom: '1px solid #E7E5E0' }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11.5, color: '#8A918D' }}>
              Операции за {p.periodLabel}{p.project ? ` · ${p.project}` : ''}
            </div>
          </div>
          <div onClick={p.onClose} className="hv-cream" style={{ width: 30, height: 30, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#8A918D' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 20px' }}>
          {loading && <div style={{ fontSize: 12.5, color: '#8A918D' }}>Загрузка…</div>}
          ...
          {rows.length > 0 && (
            <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 12, overflow: 'hidden' }}>
              {rows.map(o => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: '1px solid #F3F2ED' }}>
                  <div style={{ fontSize: 11.5, color: '#6B7370', fontFamily: PLEX, width: 78, flex: 'none' }}>{fmtD(o.date)}</div>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {o.party ?? 'Без контрагента'}
                      {o.isPlan && <span style={{ marginLeft: 6, fontSize: 10.5, fontWeight: 600, color: '#3D62B3', background: '#E7EEF9', borderRadius: 5, padding: '1px 6px' }}>план</span>}
                    </div>
                    <div style={{ fontSize: 11, color: '#A6ACA8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {[o.project, o.comment].filter(Boolean).join(' · ') || '—'}
                    </div>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', color: o.type === 'in' ? '#1A7A4B' : '#B93227', ...num }}>
                    {o.type === 'in' ? '+' : '−'}{fmt(Math.abs(o.amount))}
                  </div>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '11px 14px', fontSize: 12.5, color: '#6B7370' }}>
                <span>Показано {rows.length} из {total}</span>
                <span>Итого: <b style={{ color: '#1B1F1E', fontFamily: PLEX }}>{fmt(Math.round(sum))} TJS</b></span>
              </div>
            </div>
          )}
        </div>
