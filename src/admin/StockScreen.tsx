  ...
  const tab = (t: 'in' | 'out', label: string) => (
    <div
      ...
      style={{
        ...
        background: type === t ? '#fff' : 'transparent', color: type === t ? '#1B1F1E' : '#6B7370',
  ...
  return (
    <div data-stock-modal style={{ position: 'fixed', inset: 0, zIndex: 70 }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 460, maxWidth: '94vw', maxHeight: '92vh', overflowY: 'auto', background: '#fff', borderRadius: 14, boxShadow: '0 24px 60px rgba(0,0,0,.24)', padding: '22px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: '#EEF1EE', padding: 3, borderRadius: 9, marginBottom: 14 }}>
          {tab('in', 'Приход')}{tab('out', 'Списание')}
        </div>

        <div style={{ marginBottom: 12 }}>
          <div style={lbl}>Товар <span style={{ color: '#B93227' }}>*</span></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          <div>
            <div style={lbl}>Количество{good ? `, ${good.unit}` : ''} <span style={{ color: '#B93227' }}>*</span></div>
            <input value={amount} onChange={e => setAmount(e.target.value)} placeholder="0" style={{ ...inp, ...num }} />
            {good && type === 'out' && <div style={{ fontSize: 11, color: '#8A918D', marginTop: 3 }}>На складе: {qty(good.qty)} {good.unit}</div>}
          </div>
          <div>
...
function GoodDrawer({ item, moves, onClose, onSaved, onMove, onError }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 65 }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(21,24,23,.42)' }} />
      <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 480, maxWidth: '96vw', background: '#FBFAF7', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,.16)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 20px', background: '#fff', borderBottom: '1px solid #E7E5E0' }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11.5, color: '#8A918D' }}>{item.note || 'Товар на складе'}</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 12, padding: '14px 16px', marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.05em', color: '#8A918D', marginBottom: 10 }}>ОСТАТОК</div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-.02em', ...num }}>{qty(item.qty)}</div>
              <div style={{ fontSize: 13, color: '#6B7370' }}>{item.unit}</div>
              {item.low && <Badge b={badge('Ниже минимума', 'red')} />}
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <div onClick={() => onMove('in')} className="hv-dim" style={{ background: ACC, color: '#fff', borderRadius: 9, padding: '8px 16px', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>Приход</div>
              <div onClick={() => onMove('out')} className="hv-soft" style={{ border: '1px solid #E0DED8', borderRadius: 9, padding: '8px 16px', fontSize: 12.5, fontWeight: 600, color: '#3E4643', cursor: 'pointer' }}>Списание</div>
            </div>
          </div>

          <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 12, padding: '14px 16px', marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.05em', color: '#8A918D', marginBottom: 10 }}>ПАРАМЕТРЫ</div>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ fontSize: 11, color: '#8A918D', marginTop: 4 }}>Ниже этого значения товар помечается как «Мало» на складе и в уведомлениях.</div>
            </div>
            <div onClick={busy ? undefined : save} className="hv-dim" style={{ marginTop: 12, display: 'inline-block', background: busy ? '#B9C2BC' : ACC, color: '#fff', borderRadius: 9, padding: '8px 18px', fontSize: 12.5, fontWeight: 600, cursor: busy ? 'default' : 'pointer' }}>Сохранить</div>
          </div>

          <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.05em', color: '#8A918D', padding: '14px 16px 8px' }}>ИСТОРИЯ ДВИЖЕНИЙ</div>
            {moves.length === 0 && <div style={{ padding: '4px 16px 16px', fontSize: 12.5, color: '#8A918D' }}>Движений пока нет.</div>}
            {moves.map(m => {
              ...
              return (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderTop: '1px solid #F3F2ED' }}>
                <span style={{ width: 28, height: 28, borderRadius: 8, flex: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: m.type === 'in' ? '#E6F4EB' : '#FAE7E4', color: m.type === 'in' ? '#1A7A4B' : '#B93227', fontWeight: 700, fontSize: 13 }}>{m.type === 'in' ? '+' : '−'}</span>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 600 }}>{qty(m.qty)} {m.unit}</div>
                  <div style={{ fontSize: 11, color: '#8A918D', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {parts.join(' · ') || 'Без основания'}
                  </div>
                </div>
                <div style={{ fontSize: 11.5, color: '#6B7370', fontFamily: PLEX, flex: 'none' }}>{fmtD(m.date)}</div>
              </div>
              );
            })}
          </div>
...
export default function StockScreen({ projects, onError }: StockScreenProps) {
  ...
  const kpi = (label: string, value: string, note: string, color?: string) => (
    <div style={{ background: '#fff', border: '1px solid #E7E5E0', borderRadius: 12, padding: '13px 16px' }}>
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.05em', color: '#8A918D', marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-.02em', color: color ?? '#1B1F1E', ...num }}>{value}</div>
  ...
  return (
    <div data-screen-label="Склад">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <div style={{ fontSize: 12.5, color: '#8A918D' }}>Остатки товаров и движения по складу</div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: 12, marginBottom: 14 }}>
        ...
        {kpi('НИЖЕ МИНИМУМА', String(lowCount), lowCount ? 'нужно пополнить' : 'все остатки в норме', lowCount ? '#B93227' : '#1A7A4B')}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Поиск по названию или артикулу"
          style={{ width: isMobile ? '100%' : 260, height: 36, border: '1px solid #E0DED8', borderRadius: 9, padding: '0 12px', fontSize: 12.5, outline: 'none', background: '#fff' }} />
        <div onClick={() => setOnlyLow(v => !v)} className="hv-soft"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 7, border: '1px solid ' + (onlyLow ? ACC : '#E0DED8'), background: onlyLow ? '#E6F2EA' : '#fff', color: onlyLow ? ACC : '#3E4643', borderRadius: 9, padding: '8px 13px', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>
          Только «мало»
        </div>
      </div>
            ...
            <tbody>
              {rows.map(i => (
                <tr key={i.id} onClick={() => setSel(i.id)} className="hv-row" style={{ cursor: 'pointer' }}>
                  ...
                  <td style={{ ...cell, color: '#6B7370', fontFamily: PLEX }}>{i.sku || '—'}</td>
                  <td style={{ ...cell, textAlign: 'right', fontSize: 13, fontWeight: 600, ...num }}>{qty(i.qty)}</td>
                  <td style={{ ...cell, color: '#6B7370' }}>{i.unit}</td>
                  <td style={{ ...cell, textAlign: 'right', color: '#6B7370', ...num }}>{qty(i.minQty)}</td>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={6} style={{ padding: '26px 16px', textAlign: 'center', fontSize: 12.5, color: '#8A918D' }}>Ничего не найдено.</td></tr>
              )}
            </tbody>
          </table>
        </div>
