...
export function CheckRow({ on, label, onClick }: { on: boolean; label: string; onClick: () => void }) {
  return (
    <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '5px 0', cursor: 'pointer' }}>
      <span style={{ width: 18, height: 18, borderRadius: 5, border: `1.5px solid ${on ? ACC : '#CFCCC4'}`, background: on ? ACC : '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}>
        <svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="#fff" strokeWidth="2" style={{ opacity: on ? 1 : 0 }}><path d="M1.5 5.2L4 7.5l4.5-5" /></svg>
      </span>
...
export function Th({ style, children, right }: { style?: CSSProperties; children?: ReactNode; right?: boolean }) {
  return (
    <th style={{ padding: '8px 12px', fontSize: 10.5, fontWeight: 600, letterSpacing: '.05em', textTransform: 'uppercase', color: '#8A918D', textAlign: right ? 'right' : 'left', borderBottom: '1px solid #E7E5E0', ...style }}>
      {children}
    </th>
  );
}
...
export function AccentBtn({ children, onClick, style }: { children: ReactNode; onClick?: () => void; style?: CSSProperties }) {
  return (
    <div onClick={onClick} className="hv-dim" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: ACC, color: '#fff', borderRadius: 9, padding: '9px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', ...style }}>
      {children}
    </div>
  );
}
...
export function Logo({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={Math.round(size * 28 / 32)} viewBox="0 0 40 34" style={{ flex: 'none' }}>
      <path d="M20 1L39 33H1Z" fill="#37B15A" />
      <path d="M20 12.5L30.5 31h-21z" fill="#0E3A24" />
    </svg>
  );
}
