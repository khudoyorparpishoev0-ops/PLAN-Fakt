...
export default function FilePreview({ id, fileName, mime, onClose, onError }: FilePreviewProps) {
  ...
  return (
    <div data-file-preview style={{ position: 'fixed', inset: 0, zIndex: 90 }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 900, maxWidth: '94vw', height: '86vh', background: 'var(--fin-surface)', borderRadius: 14, boxShadow: '0 24px 60px rgba(0,0,0,.3)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 18px', borderBottom: '1px solid var(--fin-divider)' }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{fileName}</div>
            <div style={{ fontSize: 11.5, color: 'var(--fin-text-4)' }}>{type || 'файл'}</div>
          </div>
          {url && (
            <a href={url} download={fileName} className="hv-soft" style={{ border: '1px solid var(--fin-border)', borderRadius: 9, padding: '7px 13px', fontSize: 12.5, fontWeight: 600, color: ACC, textDecoration: 'none' }}>Скачать</a>
          )}
        ...
        <div style={{ flex: 1, minHeight: 0, background: 'var(--fin-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isImage ? 16 : 0 }}>
          {error && <div style={{ fontSize: 13, color: 'var(--fin-minus)', padding: 20, textAlign: 'center' }}>{error}</div>}
          {!error && !url && <div style={{ fontSize: 12.5, color: 'var(--fin-text-4)' }}>Загрузка…</div>}
          {url && isImage && <img src={url} alt={fileName} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain', borderRadius: 8, background: 'var(--fin-surface)' }} />}
          {url && isPdf && <iframe src={url} title={fileName} style={{ width: '100%', height: '100%', border: 'none' }} />}
          {url && !isImage && !isPdf && (
            <div style={{ fontSize: 12.5, color: 'var(--fin-text-2)', textAlign: 'center', padding: 20, lineHeight: 1.5 }}>
