...
export class DataController {
  ...
  /** Имена контрагентов для подсказок в формах заявок.
   *
   *  Справочник целиком бухгалтеру не положен (ТЗ 2), но подсказка при вводе —
   *  это не доступ к данным, а защита от разнописи: одобрение заявки заводит
   *  контрагента в справочник, и три «ТаджТехСнаба» с разными пробелами хуже
   *  одного. Отдаются только имена, без сумм и заметок. */
  @Get('counterparty-names')
  @Roles('admin', 'director', 'accountant')
  counterpartyNames() {
    return this.data.counterpartyNames();
  }
